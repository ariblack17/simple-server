...
'''
-------------------------------------------------------------------------------
Multi-threaded server: opens a socket on a port, listens for a client,
then waits for a wildcard query from that client once a connection is
established. Handles each client simultaneously on a separate thread,
and allows the client to send multiple queries to the server.
...
def startServer():
    ...
    serverSocket.listen(5)                      ## allow five pending client connects
